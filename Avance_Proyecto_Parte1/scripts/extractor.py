...
from db import crear_tabla, guardar_en_db  # <-- Usamos db.py corregido
    ...
    def procesar_respuesta(self, data):
        ...
        for pais in data:
            try:
                ...
                capital = pais["capital"][0] if pais.get("capital") else "N/A"
                region = pais.get("region", "N/A")
                subregion = pais.get("subregion", "N/A")
                poblacion = pais.get("population", 0)
                area = pais.get("area", 0)
                moneda = next(iter(pais.get("currencies", {})), "N/A")
                idioma = next(iter(pais.get("languages", {}).values()), "N/A")
...
if __name__ == "__main__":
    try:
        ...
        # Guardar datos en PostgreSQL usando la función de db.py
        guardar_en_db(datos)

        # Guardar JSON y CSV localmente también
        with open("data/paises_raw.json", "w", encoding="utf-8") as f:
            json.dump(datos, f, indent=2, ensure_ascii=False)
        df = pd.DataFrame(datos)
        df.to_csv("data/paises.csv", index=False, encoding="utf-8")

        logger.info("📁 Datos guardados en JSON, CSV y PostgreSQL")
    except Exception as e:
